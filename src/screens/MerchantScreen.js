...
import {sortByDateAsc, sortByDateDesc, sortByTitle} from '../store/actions/sort';

const mapStateToProps = (state, {match:{params}}) => ({
  merchant: getMerchantById(state, params.id),
  sortType: state.sortType.sortingType,
  isDialogOpen: getIsDialogOpen(state),
  filteredBids: getSortedBids(getMerchantById(state, params.id), state.sortType)
});
...
export default compose(
  ...
  connect(
    mapStateToProps,
    { setOpenDialog, onRemoveMerchant, sortByDateAsc, sortByDateDesc, sortByTitle }
  )
)(Merchant);
